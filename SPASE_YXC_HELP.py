...
#embedding_matrix_all = numpy.array(embedding_matrix).reshape(len(annotation_table),vector_size)
...
with open("result_SPASE","rb") as result:
    ...
    Sememe_embedding_Matrix = pickle.load(result)#Sememe_embedding_Matrix = Matrix_Factorization(embedding_matrix,annotation_table,sememe_size)
...
output_file = open("output_SPASE_YXC_HELP",'w')
